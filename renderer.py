...
from models.tensoRF import TensorVM, TensorCP, raw2alpha, TensorVMSplit, AlphaGridMask, TensorVMSplitRgbOnly
from utils import *
...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, device='cuda'):
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        rgb_map, depth_map, _, _, _, _, _ = tensorf(rays_chunk, is_train=is_train, white_bg=white_bg, ndc_ray=ndc_ray, N_samples=N_samples)
...
def OctreeRender_trilinear_fast_rgbonly(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, device='cuda'):
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        rgb_map = tensorf(rays_chunk, is_train=is_train, white_bg=white_bg, ndc_ray=ndc_ray,
                                     N_samples=N_samples)
...
@torch.no_grad()
def evaluation_rgbonly(test_dataset,tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
               white_bg=False, ndc_ray=False, compute_extra_metrics=True, device='cuda'):
