...
class HyperMLP(nn.Module):
    ...
    def __init__(self, z_dim = 32, in_size=256, out_size=256, K = 7):
        ...
        self.C_w = nn.Parameter(torch.rand(self.K , self.K))
        torch.nn.init.normal_(self.C_w, 0.0, np.sqrt(2) / np.sqrt(self.K))

        self.C_b = nn.Parameter(torch.rand(self.K))
...
class HypernetVanillaNeRF(BaseNeRF):
    ...
    def forward(self,ray_sampled, xyz_sampled, viewdir_sampled, z_vals, ray_valid,white_bg=True, is_train=False, ndc_ray=False):
        ...
        if ray_valid.any():
            ...
            for i,l in enumerate(self.encoder):
                w, b = self.encoder[i](self.z_space.z_list[i])
    ...
    def init_nn(self,pos_dim_pe, dir_dim_pe):
       self.z_space = Embedding(self.D + 4, self.z_dim)
       self.encoder = nn.ModuleList([HyperMLP(self.z_dim, pos_dim_pe, self.W, self.c_dim)] + \
                                    [HyperMLP(self.z_dim,self.W, self.W, self.c_dim) if (i not in [self.D // 2]) else
...
if __name__ == '__main__':
    ...
    stu_model = HypernetVanillaNeRF(**stu_args)
    ...
    rgb_map, depth_map, rgb, sigma, alpha, weight, bg_weight, sigma_feat, app_feat = stu_model(rays_chunk, xyz_chunk, viewdir_chunk, z_vals_chunk, ray_valid_chunk,is_train=True, white_bg = True, ndc_ray=0)
